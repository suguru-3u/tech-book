...
上記URLとHTTPメソッドの説明から今回のサンプルTodoアプリのURLを設計していきたいと思います。
...
まず一つ目がパフォーマンスの問題です。
実は上記のやり方では別の方面でパフォーマンスの問題が発生してしまいます。

それはpageのパラメータからDBから値を取得する際に、SQLでoffsetとlimitを使用してデータの位置を取得しようとすると、 
データの数が増えるにつれて速度が遅くなってしまうことがあることです。
...
http://api.example.com/v1/users/:id/todos?limit=50&max-id=300 (pageではなくmax-idを使用している)

発生するケースとしは、更新頻度が高いサービスとデータ量が膨大なサービスが当てはまります。
また絶対位置でデータ取得するの位置を決定する場合、事前に位置を把握するための情報を取得する必要があります。
